...
class OrderController extends Controller
{
    ...
    //             $consignmentData = [
    //                 'invoice' => $order->invoice_id,
    //                 'recipient_name' => $order->shipping->name ?? 'sabbir',
    //                 'recipient_phone' => $order->shipping->phone ?? '01751155302',
    //                 'recipient_address' => $order->shipping->address ?? 'not found',
    //                 'cod_amount' => $order->amount
    //             ];
    ...
    //     return response()->json([
    //         'status' => 'success',
    //         'message' => 'Your orders were processed',
    ...
    public function bulk_courier($slug, Request $request)
    {
        ...
        // Safely get order_ids
        $orders_ids = $request->input('order_ids', []); // default empty array if missing

        if (!is_array($orders_ids) || empty($orders_ids)) {
            return response()->json([
                'status' => 'error',
                'message' => 'No order IDs provided or invalid format.'
            ], 400);
        }
        ...
        foreach ($orders_ids as $order_id) {
            ...
            if ($order && $request->status == 5 && $order->order_status != 5) {
                ...
                $failedOrders[] = [
                    'order_id' => $order_id,
                    'message' => 'Order not found or status invalid.'
                ];
            }
        }
