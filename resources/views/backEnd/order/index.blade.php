                        ...
                        <tr>
                            ...
                            <td>
                                <button id="createSteadfastOrder" class="btn btn-primary"
                                                data-order-id="{{ $value->id }}">CSteadfast</button>
                            </td>
...
<script>
    $(document).on('click', '.multi_order_courier', function(e){
        ...
        var order = $('input.checkbox:checked').map(function(){
            return parseInt($(this).val()); // 👈 convert to integer
        });
        ...
        $.ajax({
            ...
            success: function(res){
                if(res.status === 'success'){
                    ...
                    res.success.forEach(s => {
                        toastr.success(`Order ${s.order_id}: ${s.message}`);
                    });

                    res.failed.forEach(f => {
                        toastr.error(`Order ${f.order_id}: ${f.message}`);
                    });
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const apiKey = 'bzzj8mvjcajxvvftimf0un5qgxdtkhsp';
        const secretKey = 'qb3saux5pza7fheuibkkp2kp';
        ...
        document.querySelectorAll('#createSteadfastOrder').forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                let invoiceIdCell = row.querySelector('td:nth-child(4)');
                let customerCell = row.querySelector('td:nth-child(6)');
                let phoneCell = row.querySelector('td:nth-child(7)');
                let amountCell = row.querySelector('td:nth-child(9)');

                if (!invoiceIdCell || !customerCell || !phoneCell || !amountCell) {
                    console.log('One or more table cells are missing!');
                    return;
                }

                let orderNo = invoiceIdCell.textContent.trim();
                let amountText = amountCell.textContent.trim();
                let amount = parseFloat(amountText.replace(/[^\d.-]/g, '')); // Remove ৳ symbol

                let customerName = customerCell.querySelector('strong')?.textContent.trim() || '';
                let customerAddress = customerCell.querySelector('p')?.textContent.trim() || '';
                let customerPhone = phoneCell.textContent.trim();
                ...
                fetch(`https://portal.packzy.com/api/v1/create_order`, {
                    ...
                    .then(data => {
                        if (data.status === 200) {
                            ...
                            toastr.error('Error: ' + (data.message || 'Failed to create consignment.'));
                        }
                    })
                    .catch(error => {
